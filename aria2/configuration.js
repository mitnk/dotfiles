...
.constant('$authconf', {           // default authentication configuration, never fill it in case the webui is hosted in public IP as it can be compromised
  ...
  directURL: 'https://xxx.mitnk.com/aria2/'                   // If supplied, links will be created to enable direct download from the aria2 server, requires appropriate webserver to be configured
})
.constant('$enable', {
...
.constant('$starredProps', [
    // default list of Quick Access Properties. Can be overridden
    // by making modification through the Global Settings dialog
...
.constant(
    ...
    // go to Advance Download Options when adding a new download to
    // view the list of possible options
